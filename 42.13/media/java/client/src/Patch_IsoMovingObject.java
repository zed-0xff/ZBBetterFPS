...
public class Patch_IsoMovingObject {
    ...
    public static void optimized_separate(Object selfObj) {
        ...
        int i = 0;
        while (i <= 8) {
            IsoGridSquare sq = i == 8 ? self.getCurrentSquare() : self.getCurrentSquare().getSurroundingSquares()[i];
            if (sq != null && !sq.getMovingObjects().isEmpty() && (sq == self.getCurrentSquare() || !self.getCurrentSquare().isBlockedTo(sq))) {
                ...
                for (int n = 0; n < size; n++) {
                    ...
                    if (obj != self && obj.isSolidForSeparate() && Math.abs(self.getZ() - obj.getZ()) <= 0.3f) {
                        ...
                        float twidth = self.getWidth() + obj.getWidth();
                        Vector2 diff = tempo;
                        diff.x = self.getNextX() - obj.getNextX();
                        diff.y = self.getNextY() - obj.getNextY();
                        ...
                        if (objChr == null) {
                            ...
                            if (thisPlyr != null && thisPlyr.getBumpedChr() != obj && len < twidth + maxWeaponRange && thisPlyr.getForwardDirection().angleBetween(diff) > 2.6179938155736564d && thisPlyr.getBeenSprintingFor() >= 70.0f && WeaponType.getWeaponType(thisPlyr) == WeaponType.SPEAR) {
                            ...
                            if (len < twidth) {
                                ...
                                if (GameServer.server || self.distToNearestCamCharacter() < 60.0f) {
                                    if (self instanceof IsoZombie) {
                                        ((IsoZombie) self).networkAi.wasSeparated = true;
                                    }
                                    if (self.isPushedByForSeparate(obj)) {
                                        diff.setLength((len - twidth) / 8.0f);
                                        self.setNextX(self.getNextX() - diff.x);
                                        self.setNextY(self.getNextY() - diff.y);
                                    }
